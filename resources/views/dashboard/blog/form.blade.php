<div class="card-body row">
    <div class="form-group  col-12">
        <label>عنوان المقال<span
                class="text-danger">*</span></label>
        <input name="title" placeholder="ادخل عنوان المقال" value="{{ old('title', $data->title ?? '') }}"
               class="form-control  {{ $errors->has('title') ? 'border-danger' : '' }}" type="text"
               maxlength="255"/>
    </div>
    <div class="form-group col-12">
        <label>وصف المقال<span
                class="text-danger">*</span></label>
        <div class="">
                <textarea placeholder="ادخل وصف المقال" class="form-control {{ $errors->has('description') ? 'border-danger' : '' }} "
                          name="description" rows="10" >{{ old('description', $data->description ?? '') }}</textarea>
        </div>
    </div>
    <div class="form-group col-md-6">
        <label>صورة المقال<span
                class="text-danger">*</span></label>
        <div class="col-lg-8">
